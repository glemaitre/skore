...
def _align_dataframe_backends(
    X_frame: nw.DataFrame[Any], y_frame: nw.DataFrame[Any]
) -> tuple[nw.DataFrame[Any], nw.DataFrame[Any]]:
    """Bring ``X_frame`` and ``y_frame`` onto a common dataframe backend.

    Prefer polars when the two sides differ: ``pandas -> polars`` does not
    require pyarrow, while the reverse does.
    """
    if y_frame.implementation is X_frame.implementation:
        return X_frame, y_frame
    if X_frame.implementation is nw.Implementation.POLARS:
        return X_frame, nw.from_native(y_frame.to_polars())
    return (
        nw.from_native(X_frame.to_polars()),
        nw.from_native(y_frame.to_polars()),
    )
